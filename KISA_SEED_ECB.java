...
public class KISA_SEED_ECB {
    ...
    private final static int[] NORMAL_ORDER = {0, 1, 2, 3};
    private final static int[] MIXED_ORDER = {2, 3, 0, 1};
    private static int[] rKeys = new int[32];
     ...
     * @return byte
     */
    private static byte get0Byte(int num) {
     ...
     * @return byte
     */
    private static byte get1Byte(int num) {
     ...
     * @return byte
     */
    private static byte get2Byte(int num) {
     ...
     * @return byte
     */
    private static byte get3Byte(int num) {
        return (byte) ((num >> 24) & 0x0FF);
    }

    public static int byteToInt(byte[] b, int offset, int endian) {
        if(endian == 0) {
    ...
    private static final int changeEndian(int dwS) {
        return ((((dwS << 8) | ((dwS >> 24) & 0x000000ff)) & 0x00ff00ff) | (((dwS << 24) | ((dwS >> 8) & 0x00ffffff)) & 0xff00ff00));
    }

    public static int[] charToInt(byte[] in, int inLen) {
        int[] data;
        int len, i;
        ...
        for(i=0;i<len;i++) Common.byte_to_int(data, i, in, i*4, ENDIAN);

        return data;
    }


    public static byte[] intToChar(int in[], int inLen) {
        byte[] data;
        int i;

        data = new byte[inLen];
        if(ENDIAN != 0) {
            for(i=0;i<inLen;i++) data[i] = (byte)(in[i/4] >> ((i%4)*8));
        } else {
            for(i=0;i<inLen;i++) data[i] = (byte)(in[i/4] >> ((3-(i%4))*8));
        }
     ...
     */
    private static void round(int[] temp, int[] blocks, int[] params, int subkeysNumber) {
     ...
     * @param blocks int[], szyfrowane bloki
     */
    private static void generateSubkeys(int[] temp, int[] blocks, int subkeysNumber) {
    ...
    public static void init(KISA_SEED_INFO pInfo, KISA_ENC_DEC enc, byte[] mKey) {
        int blocks[] = new int[4];
        int temp[] = new int[2];

        if( null == pInfo || null == mKey) throw new IllegalArgumentException();

        rKeys = pInfo.seed_key.key_data;										// Pointer of round keys
    ...
    public static byte[] SEED_ECB_Encrypt(byte[] mKey, byte[] pbData, int offset, int length) {
    ...
    public static byte[] SEED_ECB_Decrypt(byte[] mKey, byte[] pbData, int offset, int length) {
        ...
        init( info, KISA_ENC_DEC.KISA_DECRYPT, mKey);

        outlen = (nCipherTextLen/16)*4 ;
        outbuf = new int[outlen];
        data = charToInt(newpbszCipherText, nCipherTextLen);
    ...
    public static int SEED_ECB_Process( KISA_SEED_INFO pInfo, int[] in, int inLen, int[] out, int[] outLen ) {
        ...
        if( KISA_ENC_DEC._KISA_ENCRYPT == pInfo.encrypt ) {
            ...
            {
                KISA_SEED_Encrypt_Block_forECB( in, in_offset, out, out_offset);
            ...
            while( nCurrentCount <= inLen )
            {
                KISA_SEED_Decrypt_Block_forECB( in, in_offset, out, out_offset);
                nCurrentCount += BLOCK_SIZE_SEED;
                in_offset += BLOCK_SIZE_SEED_INT;
                out_offset += BLOCK_SIZE_SEED_INT;
            }
    ...
    public static int SEED_ECB_Close( KISA_SEED_INFO pInfo, int[] out, int out_offset, int[] outLen ) {
        ...
        if( KISA_ENC_DEC._KISA_ENCRYPT == pInfo.encrypt ) {
            ...
            KISA_SEED_Encrypt_Block_forECB( pInfo.ecb_buffer, 0, out, (out_offset)/4);
    ...
    public static class Common {
        ...
        public static void memcpy(int[] dst, int[] src, int offset, int length) {
        ...
        public static void byte_to_int(int[] dst, int dst_offset, byte[] src, int offset, int endian) {
            if(endian == 0) {
                dst[dst_offset] = ((0x0ff&src[offset]) << 24) | ((0x0ff&src[offset+1]) << 16) | ((0x0ff&src[offset+2]) << 8) | ((0x0ff&src[offset+3]));
            } else {
                dst[dst_offset] = ((0x0ff&src[offset])) | ((0x0ff&src[offset+1]) << 8) | ((0x0ff&src[offset+2]) << 16) | ((0x0ff&src[offset+3]) << 24);
            }
        }
    }
    ...
    public static void main(String[] args)
    {
        ...
        byte pbUserKey2[] = {(byte)0x47, (byte)0x06, (byte)0x48, (byte)0x08, (byte)0x51, (byte)0xE6, (byte)0x1B, (byte)0xE8, (byte)0x5D, (byte)0x74, (byte)0xBF, (byte)0xB3, (byte)0xFD, (byte)0x95, (byte)0x61, (byte)0x85};
        ...
        for (int i=0; i<16; i++)	System.out.print(Integer.toHexString(0xff&pbUserKey2[i])+" ");
        System.out.print("\n");
        System.out.print("Plaintext\t: ");
        for (int i=0; i<16; i++)	System.out.print(Integer.toHexString(0xff&pbData1[i])+" ");
        System.out.print("\n\n\n");

        pbCipher = SEED_ECB_Encrypt(pbUserKey2, pbData1, 0, 16);

        pbPlain = SEED_ECB_Decrypt(pbUserKey2, pbCipher, 0, 32);
        ...
        pbCipher = SEED_ECB_Encrypt(pbUserKey2, pbData1, 0, 2);

        pbPlain = SEED_ECB_Decrypt(pbUserKey2, pbCipher, 0, 16);
        ...
        KISA_SEED_INFO info = new KISA_SEED_INFO();
        init( info, KISA_ENC_DEC.KISA_ENCRYPT, pbUserKey);
        ...
        for (i = 0; i < PLAINTEXT_LENGTH - process_blockLeng; )
        {
            ...
            data = charToInt(pbszPlainText, process_blockLeng);
            SEED_ECB_Process( info, data, process_blockLeng, outbuf, nRetOutLeng );
            cdata = intToChar(outbuf, nRetOutLeng[0]);
        ...
        data = charToInt(pbszPlainText, remainleng);
        ...
        cdata = intToChar(outbuf, nPaddingLeng[0]);
        ...
        {
            init( info, KISA_ENC_DEC.KISA_DECRYPT, pbUserKey);
            ...
            for (i = 0; i < EncryptedMessage_length - process_blockLeng; )
            {
                ...
                data = charToInt(cipherText, process_blockLeng);
                SEED_ECB_Process( info, data, process_blockLeng, outbuf, nRetOutLeng );
                cdata = intToChar(outbuf, nRetOutLeng[0]);
            ...
            data = charToInt(cipherText, remainleng);
            SEED_ECB_Process( info, data, remainleng, outbuf, nRetOutLeng );
            if (SEED_ECB_Close( info, outbuf, nRetOutLeng[0], nPaddingLeng ) > 0 )
            {
                cdata = intToChar( outbuf, remainleng - nPaddingLeng[0] );
